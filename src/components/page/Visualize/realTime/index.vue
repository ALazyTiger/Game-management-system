<template>
  <div class="content">
    <div class="crumbs">实时数据</div>
    <div class="container">
      <el-row :gutter="20">
        <el-col :xs="12" :md="8" :lg="6" v-for="(infor, i) in inforCardData" :key="`infor-${i}`" style="height: 120px;padding-bottom: 10px;">
          <div class="content-con bg-purple">
            <div class="left-area" :style="{background: infor.color, width: leftWidth}">
              <i :class="`iconfont ${infor.icon}`" :size="36" color="#fff"></i>
            </div>
            <div class="right-area" :style="{width: rightWidth}">
              <div>
                <div class="content-outer">
                  <countTo :startVal='0' :endVal='infor.count' :duration='5000'></countTo>
                </div>
                <p>{{ infor.title }}</p>
              </div>
            </div>
      ...
      <el-table :data="dataList" border class="table" ref="multipleTable">
        <el-table-column prop="name" label="时间段" sortable></el-table-column>
